...
def avg_pool2(name, l_input, k,s=1):
    # ksize[patch,height,width,channel], strides[patch,height,width,channel]
...
def res_new(_X, _weights, _biases, _bnorm):
    ...
    #[path,16,16,512]
    pool4 = avg_pool2('pool4', conv3d, k=5, s=1)
    #[path,16,16,512]
...
with tf.Session() as sess:
    sess.run(init,feed_dict={train_phase: False})
    step = 1
    if train:
        #saver.restore(sess,"resnew-2000")
        #step=2001
        fo = open("train_ban2.txt", "w+")
        ...
        saver.restore(sess,"bn_res-15000")
